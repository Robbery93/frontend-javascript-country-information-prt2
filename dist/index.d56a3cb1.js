...
const getCurrency = (currencies)=>{
    let outcome = "";
    for(let i = 0; i < currencies.length; i++)if (i === 0) outcome = `${currencies[i].name}'s`;
    else outcome += ` and ${currencies[i].name}'s`;
    return outcome;
};
const getLanguages = (languages)=>{
    let outcome = "";
    for(let i = 0; i < languages.length; i++){
        if (i === 0) outcome += `${languages[i].name}`;
        else if (i < languages.length - 1) outcome += `, ${languages[i].name}`;
        else outcome += ` and ${languages[i].name}`;
    }
    return outcome;
};
